...
public class RESTProduct extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (id_product == -1) {
            ...
            if (data != null) {
                if (useJson) {
                    // TODO: Tarea: convertir el ProductData a json
                } else {
                    request.setAttribute("data", data);
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (id_product == -1) {
            ...
            if (useJson) {
                // parseamos la info que viene del cliente
                // obtenemos un string con toda la info del body del mensaje http (objeto json del cliente)
                JsonParser parser = Json.createParser(request.getReader());
                String key = null;
                while (parser.hasNext()) {
                    JsonParser.Event event = parser.next();
